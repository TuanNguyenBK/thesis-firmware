...
/* Private variables ---------------------------------------------------------*/
uint8_t send_data[5], send_data_temp[5],receive_data[5],echo_receive_data[1],count=0;
char* data="92345\r";
...
void DEC_ASCII(uint16_t adc, uint8_t send_data_temp[5]);
...
void DEC_ASCII(uint16_t adc, uint8_t send_data_temp[5])
{
	send_data_temp[0]=adc/100+48;
	send_data_temp[1]=(adc%100)/10+48;
	send_data_temp[2]=adc%10+48;
	send_data_temp[3]=55;
...
void HAL_TIM_PeriodElapsedCallback(TIM_HandleTypeDef *htim)
{
	if(htim->Instance==htim2.Instance)
	{
		...
		pulse=0;pulse1=0;
		Duty_Config();
//		send_data[0]=50;send_data[1]=52;
//		send_data[2]=54;send_data[3]=56;
		DEC_ASCII(adc_value*100/4096,send_data);
		HAL_UART_Transmit_IT(&huart2,send_data,5)	;
		//sample_count++;
		//Duty_Config();
