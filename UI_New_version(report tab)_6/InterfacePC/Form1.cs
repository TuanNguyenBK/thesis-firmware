...
namespace InterfacePC
{
    public partial class Form1 : Form
    {
        ...
        private void btndung_Click(object sender, EventArgs e)
        {
            ...
            lblwarn.Text = "";
            //write status
            StreamWriter write = new StreamWriter("tuan", true);
            write.WriteLine("10-02 Start  10:56      Stop  12:67099");
            write.Write("tuan");
            write.Close();
            StreamWriter write1 = new StreamWriter("tuan1", true);
            write1.WriteLine("10-02 Start  10:56      Stop  12:67099"); write1.Close();
            ////timer1.Enabled = false;
            // timer1.Interval = 50;    
        ...
        private String export_array(string a,int i)
        {
            string text,text1;
        ...
        {
            int loop = 0, i = 1;
            StreamReader read = new StreamReader("tuan");
            while (i == 1)
            {
                if (read.ReadLine() != "tuan") loop++;
                else i = 0;
            }
            label3.Text = loop.ToString();
            read.Close();
            StreamReader read1 = new StreamReader("tuan1");
            if (loop == 0)
            {
            ...
            if (loop == 1)
            {
                txtstatus1.Text = export_array(read1.ReadLine(), 1); txtstatus2.Hide(); txtstatus3.Hide();
                txtstatus4.Hide(); status2.Hide(); status3.Hide(); status4.Hide();
            }
            if (loop == 2)
            {
                txtstatus2.Text = export_array(read1.ReadLine(), 2);
                txtstatus1.Text = export_array(read1.ReadLine(), 1);
                txtstatus3.Hide(); txtstatus4.Hide(); status3.Hide(); status4.Hide();
            }
            if (loop == 3)
            {
                txtstatus3.Text = export_array(read1.ReadLine(), 3);
                txtstatus2.Text = export_array(read1.ReadLine(), 2);
                txtstatus1.Text = export_array(read1.ReadLine(), 1); txtstatus4.Hide(); status4.Hide();
            }
            ...
            if (loop >= 4)
            {
                ...
                txtstatus4.Text = export_array(read1.ReadLine(), 4);
                txtstatus3.Text = export_array(read1.ReadLine(), 3);
                txtstatus2.Text = export_array(read1.ReadLine(), 2);
                txtstatus1.Text = export_array(read1.ReadLine(), 1);
            }
